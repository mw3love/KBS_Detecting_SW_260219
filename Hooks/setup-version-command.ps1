# /version 슬래시 커맨드 설치 스크립트
# 실행 위치: KBS Peacock 프로젝트 폴더 안에서 실행해야 합니다
# 실행: powershell.exe -ExecutionPolicy Bypass -File setup-version-command.ps1

$projectDir = Get-Location
$commandsDir = Join-Path $projectDir ".claude\commands"
$commandFile = Join-Path $commandsDir "version.md"
$utf8bom = New-Object System.Text.UTF8Encoding $true

Write-Host "/version slash command setup..." -ForegroundColor Cyan
...
if (-not (Test-Path $commandsDir)) {
    ...
    Write-Host "[OK] .claude\commands folder created" -ForegroundColor Green
} else {
    Write-Host "[OK] .claude\commands folder exists" -ForegroundColor Green
}

# version.md 생성
# $ARGUMENTS 는 Claude Code 전용 변수이므로 single-quote here-string 사용
$content = @'
...
[System.IO.File]::WriteAllText($commandFile, $content, $utf8bom)
Write-Host "[OK] .claude\commands\version.md created" -ForegroundColor Green

Write-Host ""
Write-Host "Done! Type /version 1.5.6 in Claude Code to use the command." -ForegroundColor Cyan
